...
tpu_sampling_metadata = TPUSupportedSamplingMetadata(
  top_k=jax.random.randint(
    jax.random.key(17), (num_tokens,), 7, 128, dtype=jnp.int32
  ),
  top_p=jax.random.uniform(
    jax.random.key(22), (num_tokens,), dtype=jnp.float32
  ),
  temperature=10
  ** jax.random.normal(jax.random.key(73), (num_tokens,), dtype=jnp.float32),
  do_sampling=True,
)
...
logits_cases = tuple(
  jax.vmap(uniquely_define_topk)(x, tpu_sampling_metadata.top_k)
  for x in logits_cases
)

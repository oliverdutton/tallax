...
@pytest.mark.parametrize(
  ...
  [
    (8, 16),
    (8, 64),
    (8, 128),  # Within NUM_LANES
    (8, 256),
    (8, 512),
    (8, 1024),
    (8, 2048),  # Cross-lane (> NUM_LANES)
    (16, 128),
    (32, 128),
    (64, 128),
    (128, 128),
    (128, 256),  # Various batch sizes
  ],
)
...
def test_bitonic_sort_arrays(shape, dtype, descending):
  ...
  result = bitonic_sort_in_vmem(
    [arr], num_keys=1, descending=descending, interpret=interpret
  )
  sorted_arr = result[0]

  # Verify shape
  assert sorted_arr.shape == shape, (
    f"Output shape mismatch: {sorted_arr.shape} != {shape}"
  )
  ...
  for i in range(shape[0]):
    ...
    if descending:
      ...
      is_sorted = jnp.all(row[:-1] <= row[1:])
    assert is_sorted, (
      f"Row {i} is not sorted correctly (descending={descending})"
    )
...
@pytest.mark.parametrize(
  "shape", [(8, 128), (8, 2048), (16, 64), (32, 128), (128, 256)]
)
@pytest.mark.parametrize("dtype", [jnp.int32, jnp.float32])
@pytest.mark.parametrize("descending", [False, True])
...
def test_bitonic_sort_multi_key(num_keys):
  ...
  # Create test arrays
  arr1 = jax.random.randint(
    key, shape, 0, 10, dtype=jnp.int32
  )  # Primary key with duplicates
  arr2 = jax.random.randint(
    key, shape, 0, 100, dtype=jnp.int32
  )  # Secondary key

  if num_keys == 1:
    result = bitonic_sort_in_vmem(
      [arr1], num_keys=1, descending=False, interpret=interpret
    )
    ...
    result = bitonic_sort_in_vmem(
      [arr1, arr2], num_keys=2, descending=False, interpret=interpret
    )

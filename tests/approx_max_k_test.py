...
@pytest.mark.skipif(
  is_cpu_platform(), reason="approx_max_k tests require TPU/GPU"
)
def test_approx_max_k(
  dtype, k, recall_target, shape, use_lax_approx_max_k_algorithm, seed
):
  """Test approx_max_k."""
  ...
  if k == 1:
    assert recall == 1.0, (
      f"For k=1 recall must be perfect, but found {recall:.3f}"
    )

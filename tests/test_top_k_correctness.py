...
from tallax.tax.top_k import top_k
...
def tests():
  ...
  print("\nPallas:", top_k(logits, k=k, block_size=8, interpret=interpret))
  print(
  [
  (topk_xla(logits, k=k)[i] == top_k(logits, k=k, block_size=8, interpret=interpret)[i]).mean() for i in range(2)
  ]
  )

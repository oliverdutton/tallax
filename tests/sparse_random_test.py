...
def test_sparse_random_categorical(seed, axis):
    ...
    dense_masked = jnp.full((batch_dim, dense_dim), -1e12).at[indices_0, indices_1].set(sparse_logits)
    ...
    sparse_result = sparse_random_categorical(
        key, sparse_logits, [indices_0, indices_1], dim1_size=dense_masked.shape[1], axis=axis
    )[axis]

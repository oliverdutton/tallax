...
class TestRandomU48:
  ...
  def test_random_u48_with_external_callback(self):
    ...
    max_val_int = 2**40 - 12345
    max_val = U48.from_i32_array(jnp.array([max_val_int]), max_val=max_val_int)

    # Create a single index for simplicity
    dim0_indices = jnp.array([0])

    # Generate random U48 value
    result = random_u48(keys, max_val, dim0_indices)

    # Verify using external callback with arbitrary precision
    def verify_modulo(result_high, result_low, max_high, max_low, *key_data):
      """Verify the modulo operation using Python's arbitrary precision."""
      # Convert result to Python int
      result_val = (int(result_high) << 32) | int(result_low)

      # Convert max_val to Python int
      max_val_py = (int(max_high) << 32) | int(max_low)

      # Extract key data and simulate the random u128 generation
      # For verification purposes, we just check the result is in range
      assert 0 <= result_val < max_val_py, f"Result {result_val} out of range [0, {max_val_py})"

      return np.array([result_val], dtype=np.object_)

    result_u64 = result.to_u64_in_u32s()
    max_u64 = max_val.to_u64_in_u32s()

    # Extract scalar values for verification
    result_high_scalar = result_u64[0][0]
    result_low_scalar = result_u64[1][0]
    max_high_scalar = max_u64[0][0]
    max_low_scalar = max_u64[1][0]

    # Use external callback to verify
    verified = jax.pure_callback(
      verify_modulo,
      jax.ShapeDtypeStruct((1,), np.object_),
      result_high_scalar,
      result_low_scalar,
      max_high_scalar,
      max_low_scalar,
    ...
    # The callback itself does the assertion
  ...
  def test_random_u48_batched(self):
    ...
    # Multiple indices
    batch_size = 10
    dim0_indices = jnp.arange(batch_size)
  ...
  def test_random_u48_large_max_val(self):
    """Test random_u48 with a large max_val close to U48 maximum."""
    key = jax.random.key(789)
    keys = jax.random.split(key, 4)

    # Use a large value close to 48-bit maximum
    max_val_int = 2**47 - 1
    max_val = U48.from_i32_array(jnp.array([max_val_int]), max_val=max_val_int)
    ...
    result_u64 = result.to_u64_in_u32s()
    result_val = (int(result_u64[0][0]) << 32) | int(result_u64[1][0])

"""Simplified TPU lowering test that matches original config more closely."""
    ...
    *,
    ...
    for completed_m, m in zip(bins_topm_schedule, bins_topm_schedule[1:]):
        ...
        def _():
            ...
            # Termination criterion - using VMEM
    ...
    # No unconverged bins merge (guarantee_convergence=False)

    # Final top-k extraction
    global_topk_schedule = [max(x - 1, 0) for x in bins_topm_schedule[:-1]] + [
        bins_topm_schedule[-1]  # No -1 since guarantee_convergence=False
    ]
...
def time_lowering_only(fn_callable, name="function"):
    ...
    try:
        ...
        print(f"  ✓ Lowering successful: {lower_time:.2f}s ({lower_time*1000:.0f} ms)")
        ...
        print(f"  ✗ Lowering failed: {lower_time:.2f}s ({lower_time*1000:.0f} ms)")
...
def create_pallas_fn(logits, k, max_k=128, block_token=8, num_bins=256,
    ...
    """Create pallas_call matching original config: VMEM k, no guarantee_convergence."""
    ...
    print(f"  Buffer size: {buffer_size} ({max_m} * {num_bins})")
    print(f"  VMEM scratch: ({num_tokens_padded}, {buffer_size}) = {num_tokens_padded * buffer_size:,} elements")
...
def test_lowering(shape, bins_topm_unroll=64, dtype=jnp.bfloat16, seed=42):
    """Test lowering time with given unroll value."""
    num_tokens, vocab_size = shape

    print(f"\n{'#'*70}")
    print(f"Testing: shape={shape}, unroll={bins_topm_unroll}")
    print(f"{'#'*70}")
    ...
    pallas_fn = create_pallas_fn(logits, k, max_k=128, bins_topm_unroll=bins_topm_unroll)

    # Time lowering
    result = time_lowering_only(pallas_fn, name=f"topk_{shape}_unroll{bins_topm_unroll}")
...
def main():
    print("="*70)
    print("TPU Lowering Time Test - Baseline and Optimization Barriers")
    print("="*70)
    print(f"JAX version: {jax.__version__}")
    print(f"Backend: {jax.default_backend()}")
    print()

    # Test 1: Baseline with different shapes
    print("\n" + "="*70)
    print("TEST 1: Baseline lowering times")
    print("="*70)
    ...
    for shape in shapes:
        jax.clear_caches()
        baseline_results[shape] = test_lowering(shape, bins_topm_unroll=64)

    # Test 2: Reduced unrolling on small shape
    print("\n" + "="*70)
    print("TEST 2: Reduced unrolling (16, 2048)")
    print("="*70)

    unroll_results = {}
    for unroll in [32, 16]:
        jax.clear_caches()
        unroll_results[unroll] = test_lowering((16, 2048), bins_topm_unroll=unroll)
    ...
    print("\n1. Baseline Scaling:")
    if all(baseline_results[s]['success'] for s in shapes):
        t16 = baseline_results[(16, 2048)]['time']
        t256 = baseline_results[(256, 2048)]['time']
        ratio = t256 / t16
        print(f"  (16, 2048):  {t16:.1f}s")
        print(f"  (256, 2048): {t256:.1f}s")
        print(f"  Ratio: {ratio:.2f}x")
        print(f"\n  Expected ratio: ~13.2x")
        if ratio < 10:
    ...
    print("\n2. Unrolling Impact on (16, 2048):")
    baseline_64 = baseline_results[(16, 2048)]
    if baseline_64['success']:
        t64 = baseline_64['time']
        print(f"  Unroll=64: {t64:.1f}s (baseline)")
        for unroll in [32, 16]:
            if unroll in unroll_results and unroll_results[unroll]['success']:
                t = unroll_results[unroll]['time']
                speedup = t64 / t
                print(f"  Unroll={unroll:2d}: {t:.1f}s ({speedup:.2f}x speedup)")

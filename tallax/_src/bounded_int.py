"""BoundedInt: A wrapper around JAX tracers with static bound metadata.
...
class BoundedInt:
    ...
    Example:
        >>> @pl.loop(0, 10)
        >>> def loop_body(i):
        >>>     i_bounded = BoundedInt(i, lower_bound=0, upper_bound=9)
    ...
    def __repr__(self):
        return f"BoundedInt({self.value}, [{self.lower_bound}, {self.upper_bound}])"
    ...
    def __add__(self, other):
        ...
        elif isinstance(other, BoundedInt):
            return BoundedInt(
    ...
    def __pow__(self, other):
        if isinstance(other, int):
            ...
            # For 2^stage where stage is BoundedInt

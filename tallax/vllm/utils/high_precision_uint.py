...
class U48:
  ...
  def normalize(self) -> "U48":
    """Propagate carries from low part to high part."""
    mask = (1 << 24) - 1
  ...
  def sum(self, axis: int = 1, keepdims: bool = True) -> "U48":
    ...
    if any(
      ...
      raise ValueError(
        "Sum may exceed 2**31, handle in smaller sums with normalizes."
      )
  ...
  def map_reduce_sum(
    ...
    Chunks the reduction so partial sums stay within i32 as long as possible,
    then converts to U48.
...
def modulo_u128_u64(
  ...
  def body_fun(i, state):
    rh, rl = state
    bit_idx = 127 - i
    word_idx = 3 - (bit_idx >> 5)
    bit_in_word = bit_idx & 31

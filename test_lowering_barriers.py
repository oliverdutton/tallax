"""Test optimization barriers on CPU lowering."""
...
def dynamic_topk_refs_with_barriers(
    ...
    *,
    ...
    """Kernel with optimization barriers at strategic points."""
    # Initialize buffers
    ...
    # BARRIER: Prevent fusion of initialization with main loop
        ...
        for completed_m, m in zip(bins_topm_schedule, bins_topm_schedule[1:]):
            ...
            def _():
                ...
                with jax.named_scope(f"iteration_m{m}"):
                    ...
                    # BARRIER: Separate binned_topk from storage
                    bins_topm_vals = [lax.optimization_barrier(v) for v in bins_topm_vals]
                    bins_topm_idxs = [lax.optimization_barrier(v) for v in bins_topm_idxs]
                    ...
                    pivot_vals = bins_topm_vals[m - 1]
                    pivot_vals = lax.optimization_barrier(pivot_vals)

                    # Termination criterion
                    with jax.named_scope("convergence_check"):
                        pivot = pivot_vals.max(-1, keepdims=True)
                        num_larger = (
    ...
    # BARRIER: Separate main loop from final extraction
    termination_flag_ref[0] = lax.optimization_barrier(termination_flag_ref[0])
...
def test_lowering(shape, use_barriers=False, bins_topm_unroll=64, dtype=jnp.bfloat16, seed=42):
    """Test lowering time with or without barriers."""
    num_tokens, vocab_size = shape

    print(f"\n{'#'*70}")
    print(f"Testing: shape={shape}, barriers={use_barriers}, unroll={bins_topm_unroll}")
    print(f"{'#'*70}")
    ...
    # Choose kernel
    kernel_fn = dynamic_topk_refs_with_barriers if use_barriers else dynamic_topk_refs_simple

    # Create the pallas function
    pallas_fn = create_pallas_fn(logits, k, kernel_fn, bins_topm_unroll=bins_topm_unroll)

    # Time lowering
    suffix = "barriers" if use_barriers else "baseline"
    result = time_lowering_only(pallas_fn, name=f"topk_{shape}_{suffix}_unroll{bins_topm_unroll}")
...
def main():
    print("="*70)
    print("TPU Lowering Time Test - Optimization Barriers")
    print("="*70)
    ...
    # Test 1: Baseline vs Barriers
    print("\n" + "="*70)
    print("TEST 1: Baseline vs Optimization Barriers")
    print("="*70)

    jax.clear_caches()
    baseline = test_lowering(shape, use_barriers=False, bins_topm_unroll=64)

    jax.clear_caches()
    barriers = test_lowering(shape, use_barriers=True, bins_topm_unroll=64)

    # Test 2: Barriers with reduced unrolling
    print("\n" + "="*70)
    print("TEST 2: Barriers + Reduced Unrolling")
    print("="*70)

    jax.clear_caches()
    barriers_unroll32 = test_lowering(shape, use_barriers=True, bins_topm_unroll=32)

    jax.clear_caches()
    barriers_unroll16 = test_lowering(shape, use_barriers=True, bins_topm_unroll=16)
    ...
    if baseline['success'] and barriers['success']:
        t_baseline = baseline['time']
        t_barriers = barriers['time']
        speedup = t_baseline / t_barriers
        print(f"\n1. Optimization Barriers Impact:")
        print(f"  Baseline:        {t_baseline:.1f}s")
        print(f"  With barriers:   {t_barriers:.1f}s")
        print(f"  Speedup:         {speedup:.2f}x")

        if speedup > 1.2:
            print("  ✓ Barriers help significantly!")
        elif speedup > 1.05:
            print("  → Barriers help moderately")
        elif speedup > 0.95:
            print("  → No significant change")
        else:
            print("  ✗ Barriers hurt performance")

    if barriers['success']:
        t_64 = barriers['time']
        print(f"\n2. Barriers + Unrolling:")
        print(f"  Unroll=64: {t_64:.1f}s")
        if barriers_unroll32['success']:
            t_32 = barriers_unroll32['time']
            speedup_32 = t_64 / t_32
            print(f"  Unroll=32: {t_32:.1f}s ({speedup_32:.2f}x)")
        if barriers_unroll16['success']:
            t_16 = barriers_unroll16['time']
            speedup_16 = t_64 / t_16
            print(f"  Unroll=16: {t_16:.1f}s ({speedup_16:.2f}x)")

    print("\nNext steps:")
    print("- Test on larger shape (256, 2048) if these show promise")
    print("- Combine best techniques for maximum speedup")
